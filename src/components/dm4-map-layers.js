...
function createGridLayer(core) {
  ...
  // Draw horizontal lines (rows 15, 16, 17, 18, 19, 20)
  for (var j = -2; j <= 3; j++) {
    var hy = y0 + j * ch;
    var line = document.createElementNS(svgNS, "line");
  ...
  var colOriginLetter = galacticGrid.row_origin || "N";
  var rowOriginNumber = galacticGrid.col_origin || 17;
  var col0Ord = colOriginLetter.charCodeAt(0);
  
  for (var i = -2; i <= 2; i++) {
    var colLetter = String.fromCharCode(col0Ord + i);
    var vx = x0 + i * cw;
    
    for (var j = -2; j <= 3; j++) {
      var rowNumber = rowOriginNumber + j;
      var hy = y0 + j * ch;
      ...
      var cx1 = vx + cw;
      var cy1 = hy + ch;
      if (cx1 < 0 || cy1 < 0 || cx0 > width || cy0 > height) continue;
      
      var labelText = colLetter + "-" + rowNumber;
      var textEl = document.createElementNS(svgNS, "text");
      textEl.setAttribute("x", cx0 + 8);
      textEl.setAttribute("y", cy1 - 8);
...
function createRouteLayer(core) {
  ...
  // Orders route segments into a connected path for curve generation
  function buildRouteNodes(segs) {
    ...
    // Find a terminal node (degree 1) to start from
    var start = null;
    for (var node in adj) {
  ...
  // Generates smooth Catmull-Rom spline curve through points
  ...
  Object.keys(hyperlanes).forEach(function (routeName) {
    ...
    const routeClass = meta.route_class || "medium";
    const cls = routeClass === "major" ? "route-major" : "route-medium";
    
    if (!segments.length) return;
    
    // Build ordered path through route nodes
    const routeNodes = buildRouteNodes(segments);
    ...
    // Register for selection highlighting
  ...
  // Minor routes rendered as straight lines below (keep existing code unchanged)
